...
class SecondLevelPagesGatherer extends Gatherer {
  ...
  async navigateAndFetchPages(url: string): Promise<PageData[]> {
    ...
    const fetchedUrls: MunicipalitySecondLevelPages = await getSecondLevelPages(
      url,
      false,
    );

    for (const [key, primaryMenuItem] of Object.entries(primaryMenuItems)) {
      const secondLevelPagesSection: PageLink[] =
        fetchedUrls[key as keyof MunicipalitySecondLevelPages];
